...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  File? _selectedLogo;
  String? _logoUrl; // For existing remote logo
  ...
  Future<void> _fetchSetting() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Your API returns data under 'data' key? Adjust if not
  ...
  Future<void> _pickLogoImage() async {
    ...
    if (pickedImage != null) {
      setState(() {
        _selectedLogo = File(pickedImage.path);
        _logoUrl = null; // clear remote logo when picking a new local one
      });
    }
  }
  ...
  void _onDone() {
    if (_formKey.currentState!.validate()) {
      // TODO: Send form + logo update to backend here
